...
public class PreAuthCodeGrantAuthenticationProvider implements AuthenticationProvider {
  ...
  public Authentication authenticate(Authentication authentication) throws AuthenticationException {
    if (!(authentication instanceof PreAuthCodeGrantAuthenticationToken preAuthCodeGrantAuthenticationToken))
      return null;
    ...
    if (registeredClient.getAuthorizationGrantTypes().stream().noneMatch(
        grant -> PreAuthParameterNames.PRE_AUTHORIZED_CODE_GRANT.equals(grant.getValue()))) {
  ...
  private OAuth2ClientAuthenticationToken getAuthenticatedClientElseThrowInvalidClient(
    ...
    RegisteredClient registeredClient =
        this.registeredClientRepository.findByClientId(authentication.getClientId());
    if (registeredClient == null)
      throw new OAuth2AuthenticationException(OAuth2ErrorCodes.INVALID_CLIENT);

    return new OAuth2ClientAuthenticationToken(registeredClient, ClientAuthenticationMethod.NONE,
        null);
    /*
     * OAuth2ClientAuthenticationToken clientPrincipal = null; if (authentication != null &&
     * PreAuthCodeGrantAuthenticationToken.class.isAssignableFrom(authentication.getPrincipal().
     * getClass())) { clientPrincipal = (OAuth2ClientAuthenticationToken)
     * authentication.getPrincipal(); } if (clientPrincipal != null &&
     * clientPrincipal.isAuthenticated()) { return clientPrincipal; } throw new
