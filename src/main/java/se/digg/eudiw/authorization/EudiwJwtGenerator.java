...
// TODO vi borde kunna konfigurera endast en jwtCustomizer men av någon anledning har det
// krånglat...
      ...
      if (OAuth2TokenType.ACCESS_TOKEN.equals(context.getTokenType())
        ...
        if (context.getPrincipal().getPrincipal() instanceof SwedenConnectPrincipal) {
          SwedenConnectPrincipal swedenConnectPrincipal =
              (SwedenConnectPrincipal) context.getPrincipal().getPrincipal();
          claimsBuilder.claim("personalNumber",
              swedenConnectPrincipal.getSubjAttributes().getPersonalNumber());
          claimsBuilder.claim("surname", swedenConnectPrincipal.getSubjAttributes().getSurname());
          claimsBuilder.claim("givenName",
              swedenConnectPrincipal.getSubjAttributes().getGivenName());
          claimsBuilder.claim("birthDate",
              swedenConnectPrincipal.getSubjAttributes().getBirthDate());
        }

        if (context.getPrincipal() instanceof PreAuthCodeGrantAuthenticationToken) {
          SwedenConnectPrincipal swedenConnectPrincipal =
              ((PreAuthCodeGrantAuthenticationToken) context.getPrincipal())
                  .getSwedenConnectPrincipal();
          claimsBuilder.claim("personalNumber",
              swedenConnectPrincipal.getSubjAttributes().getPersonalNumber());
          claimsBuilder.claim("surname", swedenConnectPrincipal.getSubjAttributes().getSurname());
          claimsBuilder.claim("givenName",
        ...
        if (OAuth2TokenType.ACCESS_TOKEN.equals(context.getTokenType())) {
          ...
          if (AuthorizationGrantType.AUTHORIZATION_CODE
            ...
            OAuth2AuthorizationRequest authorizationRequest = (OAuth2AuthorizationRequest) context
                .getAuthorization().getAttribute(OAuth2AuthorizationRequest.class.getName());
            String nonce = null;
          ...
          } else if (AuthorizationGrantType.REFRESH_TOKEN
              .equals(context.getAuthorizationGrantType())) {
            OidcIdToken currentIdToken =
                (OidcIdToken) context.getAuthorization().getToken(OidcIdToken.class).getToken();
        ...
        if (this.jwtCustomizer != null) {
          JwtEncodingContext.Builder jwtContextBuilder =
              (JwtEncodingContext.Builder) ((JwtEncodingContext.Builder) ((JwtEncodingContext.Builder) ((JwtEncodingContext.Builder) ((JwtEncodingContext.Builder) ((JwtEncodingContext.Builder) JwtEncodingContext
