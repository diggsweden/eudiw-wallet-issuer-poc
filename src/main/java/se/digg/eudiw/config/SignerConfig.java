...
public class SignerConfig {
  ...
  public SignerConfig(@Autowired CredentialBundles credentialBundles) {
    try {
      ...
      jwsVerifier = new ECDSAVerifier(jwk.toPublicJWK().toECKey().toECPublicKey(),
          Set.of(Curve.P_256.getName()));

      jwtSigningCredential = JWTSigningCredential.builder().signer(jwsSigner).verifier(jwsVerifier)
          .supportedAlgorithms(jwsSigner.supportedJWSAlgorithms().stream().toList()).build();
