...
public class OAuth2ServerConfig {
  ...
  public SecurityFilterChain authorizationServerSecurityFilterChain(HttpSecurity http,
      ...
      @Autowired OAuth2TokenGenerator<?> tokenGenerator, @Autowired ParCacheService parCacheService,
      @Autowired CredentialOfferService credentialOfferService, DummyProofService dummyProofService)
      throws Exception {
    OAuth2AuthorizationServerConfigurer authorizationServerConfigurer =
        OAuth2AuthorizationServerConfigurer.authorizationServer();

    Consumer<OAuth2AuthorizationServerMetadata.Builder> authorizationServerMetadataCustomizer =
        new Consumer<OAuth2AuthorizationServerMetadata.Builder>() {
          @Override
          public void accept(OAuth2AuthorizationServerMetadata.Builder builder) {
            builder.claim("pushed_authorization_request_endpoint",
                String.format("%s/oauth2/par", config.getIssuerBaseUrl()));
    ...
    http
        ...
        .with(authorizationServerConfigurer, (authorizationServer) -> authorizationServer
                    ...
                    providerConfigurationEndpoint -> providerConfigurationEndpoint
                        .providerConfigurationCustomizer(builder -> builder
                            .claim("pushed_authorization_request_endpoint",
                                String.format("%s/%s", config.getIssuerBaseUrl(), "oauth2/par")))


                )) // Enable OpenID Connect 1.0
            .authorizationEndpoint(authorizationEndpoint -> authorizationEndpoint
                .authorizationRequestConverter(
                    new OAuth2ParAuthorizationCodeRequestAuthenticationConverter(parCacheService))
                .authenticationProvider(authProvider)
                .authenticationProviders(configureAuthenticationValidator())
                // .authorizationRequestConverters(converters -> converters.addFirst(new
                // ParAuthenticationConverter(parCacheService)))
            ...
            .tokenEndpoint(tokenEndpoint -> tokenEndpoint
                ...
                .accessTokenRequestConverter(new PreAuthCodeGrantAuthenticationConverter(
                    credentialOfferService, dummyProofService))
                .authenticationProvider(new PreAuthCodeGrantAuthenticationProvider(
  ...
  SecurityFilterChain defaultSecurityFilterChain(HttpSecurity http,
    ...
    http
        .addFilterBefore(
            new SwedenconnectAuthenticationReturnFilter(authenticationManager, contextRepository),
    ...
    http.sessionManagement(
        ...
        .securityContext(
            securityContext -> securityContext.securityContextRepository(contextRepository));
    return http.build();
  }
  ...
  public RegisteredClientRepository registeredClientRepository(
     ...
     * RegisteredClient.withId(UUID.randomUUID().toString()) .clientId(config.getClientId())
     * .clientAuthenticationMethods(s -> { s.add(ClientAuthenticationMethod.NONE); })
     * .authorizationGrantType(AuthorizationGrantType.AUTHORIZATION_CODE)
     * .authorizationGrantType(AuthorizationGrantType.REFRESH_TOKEN) .authorizationGrantType(new
     * AuthorizationGrantType("urn:ietf:params:oauth:grant-type:pre-authorized_code"));
     *
     *
     * registeredClientBuilder = registeredClientBuilder.redirectUri(String.format(
     * "%s/login/oauth2/code/messaging-client-pkce", config.getIssuerBaseUrl()))
     ...
     * .redirectUri("com.example.eudiwdemo:/oauthredirect") .redirectUri(config.getIssuerBaseUrl());
     *
     * for (String uri : config.getRedirectUris()) { registeredClientBuilder =
     * registeredClientBuilder.redirectUri(uri); }
     *
     * registeredClientBuilder = registeredClientBuilder.scope("identitycredential.read")
     * .scope("VerifiablePortableDocumentA1") .scope(OidcScopes.OPENID) .scope(OidcScopes.PROFILE)
     * .clientSettings(ClientSettings.builder() .requireAuthorizationConsent(true)
     * .requireProofKey(true) //Only PKCE is supported .build())
     * .tokenSettings(TokenSettings.builder() .accessTokenFormat(OAuth2TokenFormat.SELF_CONTAINED)
     * // Generate JWT token .idTokenSignatureAlgorithm(SignatureAlgorithm.RS256)
     * .accessTokenTimeToLive(Duration.ofSeconds(30 * 60))
     * .refreshTokenTimeToLive(Duration.ofSeconds(60 * 60)) .reuseRefreshTokens(true) .build())
     * .clientSettings(ClientSettings.builder().requireAuthorizationConsent(true).build());
     * RegisteredClient registeredClient = registeredClientBuilder.build(); return new
  ...
  private Consumer<List<AuthenticationProvider>> configureAuthenticationValidator() {
    return (authenticationProviders) -> authenticationProviders
