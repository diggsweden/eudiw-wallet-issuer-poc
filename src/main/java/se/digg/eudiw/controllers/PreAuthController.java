...
public class PreAuthController {
  ...
  @ResponseBody
  public String welcomeAsHTML(@PathParam("code") String code, @PathParam("state") String state)
      throws Exception {
    ...
    logger.info("callback code: %s state: %s verifier: %s nonce: %s", code, state,
        pkceVerifier.getValue(), nonce.getValue());
    QRCodeWriter qrCodeWriter = new QRCodeWriter();
    Map<String, String> map =
        Map.of("code", code, "state", state, "verifier", pkceVerifier.getValue(), "nonce",
            nonce.getValue());
  ...
  @ResponseBody
  byte[] qrCodeCallback(@PathParam("code") String code, @PathParam("state") String state)
      throws Exception {
    if (pkceVerifier == null) {
      throw new Exception("pkceVerifier is null");
    }
