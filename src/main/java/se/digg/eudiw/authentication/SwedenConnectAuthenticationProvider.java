...
@Component
public class SwedenConnectAuthenticationProvider
  ...
  public SwedenConnectAuthenticationProvider(
      @Autowired EudiwSessionSecurityContextRepository eudiwSessionSecurityContextRepository) {
  ...
  public Authentication authenticate(Authentication authentication)
    ...
    if (authentication instanceof SwedenConnectAuthenticationToken) {
      SwedenConnectAuthenticationToken authenticationToken =
          (SwedenConnectAuthenticationToken) authentication;

      if (authenticationToken.getCredentials() == null) {
        SwedenConnectAuthenticationToken token =
            new SwedenConnectAuthenticationToken(authenticationToken.getPrincipal(),
                authenticationToken.getCredentials(), authenticationToken.getAuthorities());
        token.setDetails(
      ...
      if (!ClientAuthStatus.OK.equals(authenticationToken.getCredentials().getStatus())) {
        ...
        logger.info("Authentication failed with status: {}",
            authenticationToken.getCredentials().getStatus(), authException);
        // TODO decide AuthenticationException type based on status
        throw authException;
      }
      try {
        SignedJWT signedJWT =
            idTokenValidator.validateIdToken(authenticationToken.getCredentials().getIdToken());
        IdTokenClaims idTokenClaims = null;
        if (signedJWT != null) {
          try {
            idTokenClaims = idTokenValidator.getIdTokenClaims(signedJWT);

            SwedenConnectAuthenticationToken token = new SwedenConnectAuthenticationToken(
                new SwedenConnectPrincipal(idTokenClaims.getSubjectAttributes()),
                authenticationToken.getCredentials(), List.of(new SimpleGrantedAuthority("USER")));
            token.setDetails(
                authenticationToken.getDetails());
