...
public class CredentialIssuerServiceImpl implements CredentialIssuerService {
  ...
  public CredentialIssuerServiceImpl(@Autowired EudiwConfig eudiwConfig,
      @Autowired CredentialBundles credentialBundles) {
    this.eudiwConfig = eudiwConfig;
    issuerCredential = credentialBundles.getCredential("issuercredential");
  }

  @Override
  public String credential(CredentialFormatEnum reqFormat, String credentialType,
      JWK deviceProofPublicKey, Jwt jwt) throws TokenIssuingException {
    switch (reqFormat) {
      case VC_SD_JWT -> {
  ...
  private String sdJwtVcCredential(String credentialType, JWK deviceProofPublicKey, Jwt jwt)
      throws TokenIssuingException {


    List<TokenAttribute> tokenAttributes = new ArrayList<>(Arrays.asList(
        TokenAttribute.builder().type(new TokenAttributeType("given_name"))
            .value(jwt.getClaim("givenName")).build(),
        TokenAttribute.builder().type(new TokenAttributeType("last_name"))
            .value(jwt.getClaim("surname")).build(),
        TokenAttribute.builder().type(new TokenAttributeType("family_name"))
            .value(jwt.getClaim("surname")).build(),
        TokenAttribute.builder().type(new TokenAttributeType("issuance_date")).value(new Date())
            .build(),
        TokenAttribute.builder().type(new TokenAttributeType("issuing_country")).value("SE")
            .build(),
        TokenAttribute.builder().type(new TokenAttributeType("issuing_authority")).value("DIGG")
            .build(),
        TokenAttribute.builder().type(new TokenAttributeType("expiry_date"))
            .value(Instant.now().plus(Duration.ofHours(eudiwConfig.getExpHours()))).build()));
    ...
    if (StringUtils.hasText(birthDate)) {
      ...
      try {
        ...
        tokenAttributes.addAll(
            List.of(
                TokenAttribute.builder().type(new TokenAttributeType("age_over_15"))
                    .value(ageInYears >= 15).build(),
                TokenAttribute.builder().type(new TokenAttributeType("age_over_16"))
                    .value(ageInYears >= 16).build(),
                TokenAttribute.builder().type(new TokenAttributeType("age_over_18"))
                    .value(ageInYears >= 18).build(),
                TokenAttribute.builder().type(new TokenAttributeType("age_over_20"))
                    .value(ageInYears >= 20).build(),
                TokenAttribute.builder().type(new TokenAttributeType("age_over_65"))
                    .value(ageInYears >= 65).build(),
                TokenAttribute.builder().type(new TokenAttributeType("age_in_years"))
                    .value(ageInYears).build()));
    ...
    List<TokenAttribute> tokenAttributes = new ArrayList<>(Arrays.asList(
        ...
        TokenAttribute.builder()
            .type(new TokenAttributeType(
                ...
                "expiry_date"))
            .value(
                LocalDate.ofInstant(Instant.now().plus(Duration.ofHours(eudiwConfig.getExpHours())),
      ...
      tokenAttributes.add(
          TokenAttribute.builder()
              .type(new TokenAttributeType(
                  TokenAttributeNameSpace.EUDI_WALLET_PID.getId(),
                  "birth_date"))
              .value(jwt.getClaim("birthDate")).build());
      try {
        int ageInYears = ageInYears(birthDate);
